...
export default function OfficerIssueDetailPage() {
  ...
  if (error || !issue) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <Link href="/officer/issues" style={{ color: 'var(--action-primary)', marginTop: '1rem', display: 'inline-block' }}>
          Back to Issues
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <Link href="/officer/issues" style={{ fontSize: '0.875rem', color: 'var(--action-primary)', textDecoration: 'none', display: 'inline-block', marginBottom: '1rem' }}>
        &larr; Back to Issues
      </Link>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '1 1 600px' }}>
          ...
          {issue.contradictory_evidence && issue.contradictory_evidence.length > 0 && (
            <div className="card" style={{ marginBottom: '1rem' }}>
              <h2 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.75rem', color: 'var(--warning)' }}>Contradictory Evidence</h2>
              {issue.contradictory_evidence.map((ev, i) => (
                <div key={i} style={{ padding: '0.75rem', background: '#fef3c7', borderRadius: 'var(--radius-md)', marginBottom: '0.5rem' }}>
                  <pre style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', whiteSpace: 'pre-wrap' }}>
                    {JSON.stringify(ev, null, 2)}
                  </pre>
                </div>
        ...
        <div style={{ width: '280px', flexShrink: 0 }}>
          <div className="card" style={{ marginBottom: '1rem' }}>
            <h2 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem' }}>Metrics</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Submissions</div>
                <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>{issue.raw_submission_count}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Trusted Demand</div>
                <div style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--success)' }}>{issue.trusted_demand}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Suspicious Demand</div>
                <div style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--warning)' }}>{issue.suspicious_demand}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Confidence</div>
                <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>{(issue.confidence * 100).toFixed(0)}%</div>
              </div>
              {issue.severity && (
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Severity</div>
                  <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>{issue.severity}/10</div>
                </div>
              )}
              {issue.urgency && (
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Urgency</div>
                  <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>{issue.urgency}/10</div>
                </div>
              )}
            </div>
          </div>
